...
def generate_main_menu_embed(guild):
    ...
    if vc and vc.is_connected() and song:
        ...
        q_txt = "\n".join([f"🔹 {idx+1}. {s['title'][:40]}" for idx, s in enumerate(music_queues.get(guild_id, [])[:3])])
        if q_txt:
...
# =====================================================================
# 💡 3-in-1 BRAINSTORM POLL SYSTEM (แก้ไขป้องกันตัวอักษรยาวเกิน 45 ค๊าา)
# =====================================================================
...
class LiveIdeaDropdown(discord.ui.Select):
    def __init__(self, poll_id):
        ...
        for idx, idea in enumerate(sorted_ideas):
            # 🔥 [จุดไขขานบั๊ก] ตัดประโยคแสดงผลให้อยู่ในระยะปลอดภัย ไม่เกิน 40 ตัวอักษร
...
class AddNewIdeaModal(discord.ui.Modal, title="📥 แชร์ไอเดียใหม่ของคุณเพิ่มเข้าโพลล์"):
    def __init__(self, poll_id):
        super().__init__()
        self.poll_id = poll_id
        # จำกัดความยาวฝั่งกรอกข้อมูลไว้ที่ 100 ตัว เพื่อให้การประมวลผลหลังบ้านเรียบร้อยดีค๊าา
        self.idea_input = discord.ui.TextInput(label="พิมพ์ไอเดียเด็ด ๆ ของคุณ (จะแปลงเป็นตัวเลือกโพลทันที)", placeholder="เช่น ไปพูลวิลล่าส่วนตัวแถวหัวหิน 🏡", max_length=100, required=True)
...
class LiveBrainstormPollView(discord.ui.View):
    ...
    @discord.ui.button(label="📥 พิมพ์ไอเดียเพิ่ม", style=discord.ButtonStyle.success, emoji="✨", custom_id="add_idea_btn_fixed", row=1)
    async def add_idea_btn(self, interaction: discord.Interaction, btn):
        p_id = self.poll_id
        if p_id is None:
            for item in self.children:
                if isinstance(item, LiveIdeaDropdown):
                    p_id = item.poll_id
                    break
        
        if p_id is None and interaction.message:
    ...
    @discord.ui.button(label="🏆 ปิดโพล & สรุปผลไฟนอล", style=discord.ButtonStyle.danger, emoji="📊", custom_id="close_poll_btn_fixed", row=1)
...
class MultiRoleSelectDropdown(discord.ui.Select):
    def __init__(self, guild):
        super().__init__(placeholder="🛡️ ขั้นตอนที่ 1: เลือกยศที่ต้องการแจก...", options=[discord.SelectOption(label=r.name[:40], value=str(r.id)) for r in guild.roles if r.name != "@everyone" and not r.managed][:25])
...
async def on_ready(): 
    ...
    bot.add_view(LiveBrainstormPollView())
...
async def on_message(message: discord.Message):
    ...
    if (f"doro ลบข้อความ" in lower_msg or f"doro clear" in lower_msg) and len(parts) >= 3:
        ...
        try: 
            await message.channel.purge(limit=int(parts[2]) + 1)
        except: 
            pass
        return
