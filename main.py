...
class ChannelCategorySelectView(discord.ui.View):
    def __init__(self, guild):
        super().__init__(timeout=None)
        # ให้เลือกหมวดหมู่ที่จะสร้างช่องไปไว้ในนั้น
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        # แปลงเป็น object category
        cat_obj = interaction.guild.get_channel(int(self.category_select.values[0]))
        await interaction.response.send_modal(MultiChannelModal(category=cat_obj))
...
class BotCommandControlSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # 🏗️ เพิ่มส่วนนี้: เรียกดูหมวดหมู่ก่อนสร้างห้องแชท
        if value == "setup_channels":
            embed = discord.Embed(
                title="🏗️ โหมดสร้างห้องแชทหลายห้อง",
                description="เลือกหมวดหมู่ที่ต้องการให้ห้องแชทไปอยู่ก่อนนะค๊าา~ 🎀",
                color=0xF1C40F
            )
